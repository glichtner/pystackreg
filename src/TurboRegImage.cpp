...
/*********************************************************************
 Converts the pixel array of the incoming <code>ImagePlus</code>
    ...
    ********************************************************************/
TurboRegImage::TurboRegImage(double *img, int width, int height,
...
void TurboRegImage::antiSymmetricFirMirrorOffBounds1D(std::vector<double> &h,
                                                      std::vector<double> &c,
                                                      std::vector<double> &s) {
  if (2 <= h.size()) {
    s[0] = h[1] * (c[1] - c[0]);
...
/*------------------------------------------------------------------*/
void TurboRegImage::basicToCardinal2D(const std::vector<double> &basic,
                                      std::vector<double> &cardinal, int width,
...
void TurboRegImage::buildCoefficientPyramid() {
  ...
  for (int depth = 1; ((depth < pyramidDepth)); depth++) {
    ...
    halfWidth /= 2;
    halfHeight /= 2;
    std::vector<double> halfDual =
        getHalfDual2D(fullDual, fullWidth, fullHeight);

    ImageStackItem stackItem(halfWidth, halfHeight, false);

    // std::vector<double> halfCoefficient =
    getBasicFromCardinal2D(halfDual, halfWidth, halfHeight, 7,
                           stackItem.halfImg);
...
void TurboRegImage::buildImageAndGradientPyramid() {
  ...
  for (int depth = 1; ((depth < pyramidDepth)); depth++) {
    ...
    coefficientToXYGradient2D(stackItem.halfImg, stackItem.xGradient,
                              stackItem.yGradient, halfWidth, halfHeight);
    basicToCardinal2D(stackItem.halfImg, stackItem.halfImg, halfWidth,
                      halfHeight, 3);
...
/*------------------------------------------------------------------*/
void TurboRegImage::cardinalToDual2D(std::vector<double> &cardinal,
                                     std::vector<double> &dual, int width,
                                     int height, int degree) {
  std::vector<double> basic(width * height);
  basicToCardinal2D(
      getBasicFromCardinal2D(cardinal, width, height, degree, basic), dual,
      width, height, 2 * degree + 1);
} /* end cardinalToDual2D */

/*------------------------------------------------------------------*/
void TurboRegImage::coefficientToGradient1D(std::vector<double> &c) {
#ifdef _MSC_VER
...
/*------------------------------------------------------------------*/
void TurboRegImage::coefficientToSamples1D(std::vector<double> &c) {
#ifdef _MSC_VER
...
/*------------------------------------------------------------------*/
void TurboRegImage::coefficientToXYGradient2D(std::vector<double> &basic,
...
/*------------------------------------------------------------------*/
void TurboRegImage::dualToCardinal2D(std::vector<double> &dual,
                                     std::vector<double> &cardinal, int width,
                                     int height, int degree) {
  std::vector<double> basic(width * height);
  basicToCardinal2D(
...
/*------------------------------------------------------------------*/
void TurboRegImage::extractColumn(std::vector<double> &array, int width, int x,
...
/*------------------------------------------------------------------*/
void TurboRegImage::extractRow(const std::vector<double> &array, int y,
...
/*------------------------------------------------------------------*/
std::vector<double>
TurboRegImage::getBasicFromCardinal2D(std::vector<double> &cardinal, int width,
                                      int height, int degree,
...
/*------------------------------------------------------------------*/
std::vector<double> TurboRegImage::getHalfDual2D(std::vector<double> &fullDual,
                                                 int fullWidth,
                                                 int fullHeight) {
...
double TurboRegImage::getInitialAntiCausalCoefficientMirrorOffBounds(
    std::vector<double> &c, double z, double tolerance) {
...
/*------------------------------------------------------------------*/
void TurboRegImage::putColumn(std::vector<double> &array, int width, int x,
                              std::vector<double> &column) {
  for (unsigned int i = 0; (i < column.size()); i++) {
...
/*------------------------------------------------------------------*/
void TurboRegImage::putRow(std::vector<double> &array, int y,
                           std::vector<double> &row) {
  y *= row.size();
  for (unsigned int i = 0; (i < row.size()); i++) {
    array[y++] = (float)row[i];
...
/*------------------------------------------------------------------*/
void TurboRegImage::reduceDual1D(std::vector<double> &c,
                                 std::vector<double> &s) {
...
/*------------------------------------------------------------------*/
void TurboRegImage::samplesToInterpolationCoefficient1D(std::vector<double> &c,
                                                        int degree,
                                                        double tolerance) {
  std::vector<double> z;
  double lambda = 1.0;
  ...
  for (unsigned int k = 0; (k < z.size()); k++) {
    ...
    c[c.size() - 1] =
        getInitialAntiCausalCoefficientMirrorOffBounds(c, z[k], tolerance);
...
/*------------------------------------------------------------------*/
void TurboRegImage::symmetricFirMirrorOffBounds1D(std::vector<double> &h,
                                                  std::vector<double> &c,
                                                  std::vector<double> &s) {
  switch (h.size()) {
  case 2: {
    if (2 <= c.size()) {
      ...
      s[s.size() - 1] =
          h[0] * c[c.size() - 1] + h[1] * (c[c.size() - 2] + c[c.size() - 1]);
    } else {
      s[0] = (h[0] + 2.0 * h[1]) * c[0];
    }
    break;
    ...
    if (6 <= c.size()) {
      ...
      case 5: {
        ...
        s[1] = h[0] * c[1] + h[1] * (c[0] + c[2]) + h[2] * (c[0] + c[3]) +
               h[3] * (c[1] + c[4]);
        s[2] =
            h[0] * c[2] + h[1] * (c[1] + c[3]) + (h[2] + h[3]) * (c[0] + c[4]);
