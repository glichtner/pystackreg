...
TurboRegTransform::TurboRegTransform(
    TurboRegImage *sourceImg, TurboRegMask *sourceMsk,
    TurboRegPointHandler *sourcePh, TurboRegImage *targetImg,
    TurboRegMask *targetMsk, TurboRegPointHandler *targetPh,
    Transformation transformation, bool accelerated)
    : accelerated(accelerated), dxWeight(4), dyWeight(4), xWeight(4),
      yWeight(4), xIndex(4), yIndex(4), transformation(transformation),
      sourceImg(sourceImg), targetImg(targetImg), sourceMsk(sourceMsk),
      targetMsk(targetMsk), sourcePh(sourcePh), bHasSourceMask(true) {
...
TurboRegTransform::TurboRegTransform(TurboRegImage *sourceImg,
    ...
    : accelerated(accelerated), dxWeight(4), dyWeight(4), xWeight(4),
      yWeight(4), xIndex(4), yIndex(4), transformation(transformation),
      sourceImg(sourceImg), sourceMsk(sourceMsk), sourcePh(sourcePh),
      bHasSourceMask(true) {
  pixelPrecision = PIXEL_HIGH_PRECISION;
  maxIterations = MANY_ITERATIONS;
...
/*********************************************************************
 Append the current landmarks into a text file. Rigid format.
    ...
    ********************************************************************/
void TurboRegTransform::appendTransformation(std::string pathAndFilename) {
...
/*********************************************************************
 Compute the image.
    ********************************************************************/
void TurboRegTransform::doBatchFinalTransform(std::vector<double> &pixels) {
  if (accelerated) {
...
/*********************************************************************
 Compute the image.
    ********************************************************************/
std::vector<double> TurboRegTransform::doFinalTransform(int width, int height) {
  if (accelerated) {
    inImg = sourceImg->getImage();
  } else {
    inImg = sourceImg->getCoefficient();
...
std::vector<double> TurboRegTransform::doFinalTransform(
    TurboRegImage *sourceImg, TurboRegPointHandler *sourcePh,
    TurboRegImage *targetImg, TurboRegPointHandler *targetPh,
    Transformation transformation, bool accelerated) {
...
std::vector<double>
TurboRegTransform::doFinalTransform(TurboRegImage *sourceImg,
                                    matrix<double> &m) {
  this->sourceImg = sourceImg;
  if (accelerated) {
    inImg = sourceImg->getImage();
...
void TurboRegTransform::doRegistration() {
  ...
  iterationPower =
      (int)pow((double)ITERATION_PROGRESSION, (double)pyramidDepth);
  ...
  while (!targetImgPyramid.empty()) {
    ...
    if (transformation == BILINEAR) {
      inNx =
          srcImgItem.halfWidth; //((Integer)sourceImgPyramid.pop()).intValue();
      inNy =
          srcImgItem.halfHeight; //((Integer)sourceImgPyramid.pop()).intValue();
      ...
      outNx =
          tarImgItem.halfWidth; //((Integer)targetImgPyramid.pop()).intValue();
      outNy =
          tarImgItem.halfHeight; //((Integer)targetImgPyramid.pop()).intValue();
      ...
      outNx =
          srcImgItem.halfWidth; //((Integer)sourceImgPyramid.pop()).intValue();
      outNy = srcImgItem
                  .halfHeight; //(((Integer)sourceImgPyramid.pop()).intValue();
...
....................................................................*/
/*------------------------------------------------------------------*/
void TurboRegTransform::affineTransform(matrix<double> &m) {
  double yx;
  double yy;
  double x0;
  double y0;
...
/*------------------------------------------------------------------*/
void TurboRegTransform::bilinearTransform(matrix<double> &m) {
  double yx;
  double yy;
  double yxy;
  double yyy;
...
void TurboRegTransform::computeBilinearGradientConstants() {
  ...
  double det = uv12 * v34 - uv13 * v24 + uv14 * v23 + uv23 * v14 - uv24 * v13 +
               uv34 * v12;
...
/*------------------------------------------------------------------*/
double TurboRegTransform::getAffineMeanSquares(matrix<double> &sourcePoint,
                                               matrix<double> &m) {
...
/*------------------------------------------------------------------*/
double TurboRegTransform::getBilinearMeanSquares(matrix<double> &m) {
  double yx;
  double yy;
  double yxy;
  double yyy;
...
double TurboRegTransform::getBilinearMeanSquares(
    matrix<double> &m, matrix<double> &hessian, std::vector<double> &gradient) {
  double yx;
  double yy;
  double yxy;
  double yyy;
...
/*------------------------------------------------------------------*/
double TurboRegTransform::getRigidBodyMeanSquares(matrix<double> &m) {
  double yx;
  double yy;
  double x0;
  double y0;
...
/*------------------------------------------------------------------*/
double
TurboRegTransform::getScaledRotationMeanSquares(matrix<double> &sourcePoint,
                                                matrix<double> &m) {
  double u1 = sourcePoint(0, 0);
  double u2 = sourcePoint(1, 0);
  double v1 = sourcePoint(0, 1);
...
double TurboRegTransform::getScaledRotationMeanSquares(
    matrix<double> &sourcePoint, matrix<double> &m, matrix<double> &hessian,
    std::vector<double> &gradient) {
  double u1 = sourcePoint(0, 0);
  double u2 = sourcePoint(1, 0);
  double v1 = sourcePoint(0, 1);
...
/*------------------------------------------------------------------*/
matrix<double>
TurboRegTransform::getTransformationMatrix(matrix<double> &fromCoord,
  ...
  case RIGID_BODY: {
    double angle =
        atan2(fromCoord(2, 0) - fromCoord(1, 0),
              fromCoord(2, 1) - fromCoord(1, 1)) -
        atan2(toCoord(2, 0) - toCoord(1, 0), toCoord(2, 1) - toCoord(1, 1));
    double c = cos(angle);
    double s = sin(angle);
    // matrix = new double[2][3];
    m.resize(2, 3);
    m(0, 0) = toCoord(0, 0) - c * fromCoord(0, 0) + s * fromCoord(0, 1);
    m(0, 1) = c;
    m(0, 2) = -s;
    m(1, 0) = toCoord(0, 1) - s * fromCoord(0, 0) - c * fromCoord(0, 1);
    m(1, 1) = s;
    m(1, 2) = c;
    break;
  }
...
/*------------------------------------------------------------------*/
double TurboRegTransform::getTranslationMeanSquares(matrix<double> &m) {
  double dx = m(0, 0);
  double dy = m(1, 0);
  double dx0 = dx;
  double difference;
...
/*------------------------------------------------------------------*/
double
TurboRegTransform::getTranslationMeanSquares(matrix<double> &m,
                                             std::vector<double> &gradient) {
  double dx = m(0, 0);
  double dy = m(1, 0);
  double dx0 = dx;
...
void TurboRegTransform::inverseMarquardtLevenbergOptimization() {
  matrix<double> attempt(transformation / 2,
                         2); // = new double[transformation / 2][2];
  matrix<double> hessian(
      transformation,
      transformation); // = new double[transformation][transformation];
  matrix<double> pseudoHessian(
      transformation,
      transformation); // = new double[transformation][transformation];
  ...
  switch (transformation) {
  case TRANSLATION: {
    bestMeanSquares = getTranslationMeanSquares(m, hessian, gradient);
    break;
  }
  case SCALED_ROTATION: {
    bestMeanSquares =
        getScaledRotationMeanSquares(sourcePoint, m, hessian, gradient);
    break;
  }
  case AFFINE: {
    bestMeanSquares = getAffineMeanSquares(sourcePoint, m, hessian, gradient);
    break;
  }
  default:
    break;
  ...
  do {
    ...
    case TRANSLATION: {
      if (accelerated) {
        meanSquares = getTranslationMeanSquares(m, gradient);
      } else {
        meanSquares = getTranslationMeanSquares(m, hessian, gradient);
      }
      break;
    }
    case SCALED_ROTATION: {
      if (accelerated) {
        meanSquares = getScaledRotationMeanSquares(attempt, m, gradient);
      } else {
        meanSquares =
            getScaledRotationMeanSquares(attempt, m, hessian, gradient);
      }
      break;
    }
    case AFFINE: {
      if (accelerated) {
        meanSquares = getAffineMeanSquares(attempt, m, gradient);
      } else {
        meanSquares = getAffineMeanSquares(attempt, m, hessian, gradient);
      }
      break;
    }
    default:
...
void TurboRegTransform::inverseMarquardtLevenbergRigidBodyOptimization() {
  ...
  for (int k = 0; (k < transformation); k++) {
    sourcePoint(k, 0) =
        m(0, 0) + targetPoint(k, 0) * m(0, 1) + targetPoint(k, 1) * m(0, 2);
    sourcePoint(k, 1) =
        m(1, 0) + targetPoint(k, 0) * m(1, 1) + targetPoint(k, 1) * m(1, 2);
  }
...
/*------------------------------------------------------------------*/
void TurboRegTransform::invertGauss(matrix<double> &m) {
  int n = m.nrows();
  ...
  for (int j = 0; (j < n); j++) {
    ...
    if (k != j) {
      ...
      std::vector<double> fullLine(inverse.getRowPtr(j),
                                   inverse.getRowPtr(j + 1));

      // System.arraycopy(inverse[k], 0, inverse[j], 0, n);
      // copy inverse[k][0->n] to inverse[j][0->n]
      std::copy(inverse.getRowPtr(k), inverse.getRowPtr(k + 1),
                inverse.getRowPtr(j));
...
/*------------------------------------------------------------------*/
std::vector<double>
TurboRegTransform::matrixMultiply(matrix<double> &m,
...
/*------------------------------------------------------------------*/
void TurboRegTransform::translationTransform(matrix<double> &m) {
  double dx = m(0, 0);
  double dy = m(1, 0);
  double dx0 = dx;
  int xMsk;
