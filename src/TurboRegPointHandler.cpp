...
TurboRegPointHandler::TurboRegPointHandler(TurboRegImage &img,
                                           Transformation transformation)
    : precisionPoint() {
  setPointsByTransformation(img.getWidth(), img.getHeight(), transformation);
};

TurboRegPointHandler::TurboRegPointHandler(matrix<double> &precisionPoint)
    : precisionPoint(precisionPoint){

      };

void TurboRegPointHandler::setPointsByTransformation(
    int width, int height, Transformation transformation) {
  // increment width and height because images are supplied cropped by 1 px from
  // python (this cropping is exactly what StackReg does when calling TurboReg)
  width++;
  height++;
  ...
  case BILINEAR: {
    ...
    precisionPoint(1, 0) = (floor(0.25 * GOLDEN_RATIO * (double)width));
    precisionPoint(1, 1) =
        height - (ceil(0.25 * GOLDEN_RATIO * (double)height));
    precisionPoint(2, 0) = width - (ceil(0.25 * GOLDEN_RATIO * (double)width));
...
void TurboRegPointHandler::setPoints(matrix<double> &precisionPoint) {
